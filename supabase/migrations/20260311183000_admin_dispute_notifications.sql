...
BEGIN
  ...
  SELECT d.id, d.status, d.transaction_id, d.buyer_id, d.seller_id
  INTO v_dispute
  ...
  IF to_regclass('public.notifications') IS NOT NULL THEN
    ...
    IF v_dispute.buyer_id IS NOT NULL THEN
      ...
      VALUES (
        v_dispute.buyer_id,
        'system',
        'STATUS DA DISPUTA ATUALIZADO',
        COALESCE('Item: ' || COALESCE(v_item_title, 'Item') || ' • Novo status: ' || v_status, 'Novo status: ' || v_status),
        v_item_id,
        p_dispute_id,
        'dispute:status:' || p_dispute_id::text || ':' || v_status || ':' || v_dispute.buyer_id::text,
        false,
        now()
      )
    ...
    IF v_dispute.seller_id IS NOT NULL THEN
      ...
      VALUES (
        ...
        v_item_id,
        p_dispute_id,
        'dispute:status:' || p_dispute_id::text || ':' || v_status || ':' || v_dispute.seller_id::text,
        false,
        now()
      )
      ON CONFLICT (user_id, dedupe_key) DO NOTHING;
...
BEGIN
  ...
  ELSIF v_resolution = 'release' THEN
    ...
    END IF;
  ELSE
    v_final_status := 'resolved_refund';
    UPDATE public.disputes
    SET status = v_final_status,
        admin_id = v_uid,
        resolved_at = now(),
  ...
  IF to_regclass('public.notifications') IS NOT NULL THEN
    IF v_dispute.buyer_id IS NOT NULL THEN
      ...
      VALUES (
        ...
        'system',
        'DISPUTA RESOLVIDA',
        COALESCE('Item: ' || COALESCE(v_item_title, 'Item') || ' • Resultado: ' || v_final_status, 'Resultado: ' || v_final_status),
        v_item_id,
        p_dispute_id,
