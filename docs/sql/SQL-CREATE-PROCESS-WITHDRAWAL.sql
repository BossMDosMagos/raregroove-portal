-- =====================================================================
-- FUNÇÃO: Processar Saque (Admin)
-- =====================================================================
-- Permite administrador aprovar ou rejeitar solicitações de saque

-- 1. Criar função para processar saque
CREATE OR REPLACE FUNCTION process_withdrawal(
  withdrawal_uuid uuid,
  new_status text, -- 'concluido' ou 'cancelado'
...
BEGIN
  ...
  -- Se aprovar, verificar saldo novamente
  IF new_status = 'concluido' THEN
    ...
    -- Validar que os comprovantes foram enviados
    ...
    -- Atualizar saque como concluído
    UPDATE withdrawals
    SET 
      status = 'concluido',
      processed_at = now(),
    ...
    -- Notificação para o usuário dono da solicitação
  ...
  ELSE
    ...
    -- Registrar cancelamento no ledger
    INSERT INTO financial_ledger (
      source_type,
      source_id,
      entry_type,
...
-- 2. Criar política RLS para admin ver todos os saques
DROP POLICY IF EXISTS "Admins veem todos os saques" ON withdrawals;
CREATE POLICY "Admins veem todos os saques"
  ON withdrawals
  FOR SELECT
  TO authenticated
  USING (
...
SELECT '✅ Função process_withdrawal criada com sucesso!' AS status;

-- 4. Teste: Ver saques pendentes
SELECT 
  w.id,
  w.amount,
  w.pix_key,
